...
Eigen::MatrixXf Tracker::joint_probability(
  const Matrices& _association_matrices,
  const Vec2f& selected_detections)
{
  const auto hyp_num = _association_matrices.size();
  Eigen::VectorXf Pr(_association_matrices.size());
  const uint validationIdx = _association_matrices.at(0).rows();
  const auto tracksize = tracks_.size();
  float prior;
  ...
  for (uint i = 0; i < hyp_num; ++i)
  {
    ...
    for (uint j = 0; j < validationIdx; ++j)
    {
      ...
      const auto& mea_indicator = static_cast<int>(A_matrix.sum());
      ///////////////////////////////////////////////
      ...
      if (mea_indicator == 1)
      {
        ...
        for (uint notZero = 0; notZero < tracksize; ++notZero)
        {
          ...
          {
            const auto& z_predict = tracks_.at(notZero)->getLastPredictionEigen();
            const auto& S = tracks_.at(notZero)->S();
            cv::Mat S_cv;
            cv::eigen2cv(S, S_cv);
    ...
    const auto& likelihood = N / float(std::pow(V, false_alarms));
    ...
    else
    {
      ...
      for (uint j = 0; j < tracksize; ++j)
      {
        ...
        const int& target_indicator = static_cast<int>(target_matrix.sum());
    ...
    auto a = 1;
    for (auto j = 1; j <= false_alarms; ++j)
    {
      a = a * j;
    }

    Pr(i) = a * likelihood * prior;
  }

  const auto& prSum = Pr.sum();
...
Tracker::Matrices Tracker::generate_hypothesis(
  const Vec2f&,
  const cv::Mat& _q) const
{
  const uint validationIdx = _q.rows;
  ...
  std::copy(
    tmp_association_matrices.begin(),
    tmp_association_matrices.begin() + hyp_num + 1,
    association_matrices.begin());
  return association_matrices;
}


Tracker::VecBool Tracker::analyze_tracks(const cv::Mat& _q, const std::vector<Detection>&) const
{
  const auto& m_q = _q(cv::Rect(1, 0, _q.cols - 1, _q.rows));
